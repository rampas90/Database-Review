...
* 이때 설정할 수 있는 동작은 다음과 같습니다.
  ...
  * 예제
    ...
    ```
    다음 예제는 RESERVATION_PARENT 테이블의 PARENTID 필드에 RESERVATION_CHILD 테이블의 ID 필드를 참조하는 FOREIGN KEY 제약 조건을 설정하는 예제이다. 이때 참조되는 필드의 데이터가 수정될 때는 CASCADE 방식으로, 삭제될 때는 RESTRICT 방식으로 동작하게 된다.
