...
* 예제
  ...
  FROM TEST;
  ```
  풀어서 말하면, **'TEST 테이블로부터 모든 필드를 선택하겠다.'** 라는 뜻이다.
...
* 예제1
  ...
  WHERE NAME = '홍길동';
  ```
  풀어서 말하면, **'TEST 테이블로부터 NAME이 홍길동인 사람 중에서 모든 필드를 선택하겠다.'** 라는 뜻이다.

* <img src="../../images/2_12.PNG" width="600"/>

* 이러한 WHERE 절에는 여러 개의 조건을 같이 명시할 수도 있다. 이때 여러 개의 조건은 AND나 OR 연산자를 이용하여 연결한다.
...
* 예제2
  ...
  ```
  풀어서 말하면, **'TEST 테이블로부터 ID값이 3이하이면서 RESERVEDATE 필드의 값이 2016년 2월 1일 이후인 레코드만 선택하겠다.'** 라는 뜻이다.
...
* SELECT 키워드 다음에 필드 이름을 명시하면, 해당 테이블의 특정 필드만을 불러올 수 있다. 이때 쉼표(,)를 사용하여 여러 개의 필드 이름을 한 번에 명시할 수 있다.
...
* 예제1
  ...
  FROM TEST;
  ```
  풀어서 말하면, **'TEST 테이블로부터 NAME과 ROOMNUM의 필드값만 선택하겠다.'** 라는 뜻이다.

* <img src="../../images/2_14.PNG" width="600"/>

* 예제2
  * 이때에도 WHERE 문을 사용하여 특정 조건을 마족하는 레코드만을 선택할 수 있다.
  ```SQL
  SELECT NAME, RESERVEDATE
  FROM TEST
  WHERE ID <= 3 AND RESERVEDATE > '2016-02-01';
  ```
  풀어서 말하면, **'TEST 테이블로부터 ID 값이 3이하이면서 RESERVEDATE 필드의 값이 2016년 2월 1일 이후인 레코드의 NAME 필드와 RESERVEDATE 필드만을 선택하겠다.'** 라는 뜻이다.
...
* 예제
  ...
  FROM TEST;
  ```
  풀어서 말하면, **'TEST 테이블로부터 DISTINCT(분명한, 뚜렷한) NAME 즉, 중복된 값은 단 한 번만 선택하겠다.'** 라는 뜻이다.
...
* SELECT 문으로 선택한 결과를 **ORDER BY 절** 을 사용하여 정렬할 수 있다. ORDER BY 절은 기본 설정은 오름차순이며, ASC 키워드를 사용하여 직접 오른차순을 명시할 수도 있다.
...
* 예제1
  ...
  ```
  풀어서 말하면, **'TEST 테이블로부터 RESERVEDATE필드의 오름차순으로 정렬하여 선택하겠다.'** 라는 뜻이다.
...
* 예제2
  ...
  ```
  풀어서 말하면, **'TEST 테이블로부터 RESERVEDATE필드의 내림차순으로 정렬하여 선택하겠다.'** 라는 뜻이다.

* <img src="../../images/2_18.PNG" width="600"/>
***

* PHP에서 문자역 타입을 데이터로 가지는 필드를 정렬할 때는 기본적으로 대소문자를 구분하지 않는다. 하지만 대소문자까지 구분하여 정렬하고 싶을 때는 ORDER BY BINARY 절을 사용하면 된다.

* 여러 필드의 데이터를 쉼표(,)를 사용하여 한 번에 정렬할 수도 있다.
...
* 예제3
  ...
  ```
  풀어서 말하면, **'TEST 테이블로부터 RESERVEDATE필드의 내림차순으로 정렬한 뒤에, 또다시 ROOMNUM 필드의 내림차순으로 정렬하여 선택하겠다.'** 라는 뜻이다.
...
* 예제
  ...
  FROM TEST;
  ```
  풀어서 말하면, **'TEST 테이블로부터 RESERVEINFO라는 별칭으로 ROOMNUM:NAME로 결합하여 하나의 문자열로 반환하는 RESERVEDATE모든 필드를 선택하겠다.'** 라는 뜻이다.
